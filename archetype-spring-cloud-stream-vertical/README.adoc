...
Los módulos verticales principales son `player` y `confirmation`, cada uno con su propia estructura interna siguiendo Clean Architecture.
Dentro de cada módulo encontraremos:
...
Resumen de las carpetas y su responsabilidad principal:

- `src/main/java/org/labcabrera/sample/archetype` — paquete base del proyecto.
    - `player/` — módulo vertical para la funcionalidad de jugador.
        - `application/` — casos de uso y servicios (orquestación, validación, reglas de negocio).
            - `cqrs/` — handlers y comandos/consultas.
            - `services/` — servicios de aplicación (ej. CreatePlayerService, UpdatePlayerService).
            - `ports/` — interfaces (puertos) hacia infraestructuras (repositorios, eventbus).
        - `domain/` — entidades de dominio, value objects y enums (ej. `Player`, `PlayerStatus`).
        - `infrastructure/` — adaptadores (JPA, Kafka, mappers). Ej: `persistence/jpa` contiene entidades JPA y adaptadores que implementan los puertos.
        - `interfaces/` — adaptadores de entrada (HTTP controllers, Spring Cloud Stream Consumers/Producers). Ej: `kafka/KafkaPlayerController`.
- `confirmation/` — módulo para la lógica de confirmaciones de email; estructura similar al módulo `player`.
- `shared/` — código compartido: excepciones, utilidades, bus de comandos simple, modelos comunes.

TIP: El contenido del módulo `shared/` podría extraerse a módulos externos autoconfigurables equivalentes a los starters de Spring Boot para compartir entre proyectos.
...
- Spring Boot 3.x (Java 21 toolchain)
...
Se ha integrado `springdoc-openapi` para generar documentación OpenAPI/Swagger automáticamente a partir de los controladores REST. Esto facilita la exploración y prueba de la API durante el desarrollo.
